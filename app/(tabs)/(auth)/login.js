...
import { login as loginApi, getProfile, adminGetSellers, getSellerMe } from '../../../shared/api';
...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (isNetwork) msg = 'Сервер недоступний. Перевірте підключення до мережі.';
      else if (status === 401 || status === 400) msg = 'Невірний email або пароль';
      else if (status >= 500) msg = `Помилка сервера (${status}). Спробуйте пізніше.`;
