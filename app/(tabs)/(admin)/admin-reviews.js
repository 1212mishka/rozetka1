...
export default function AdminReviews() {
  ...
  const doAction = async (id, action) => {
    ...
    try {
      ...
      const msg = e?.response?.data?.detail || 'Помилка виконання дії';
  ...
  const handleDelete = (id) => {
    Alert.alert('Видалити відгук?', 'Цю дію не можна скасувати', [
      { text: 'Скасувати', style: 'cancel' },
      { text: 'Видалити', style: 'destructive', onPress: () => doAction(id, adminDeleteProductReview) },
    ]);
  };
  ...
  const renderReview = ({ item }) => {
    ...
    const isLoading = actionId === itemId;
    return (
      <View style={styles.card}>
        <View style={styles.cardTop}>
          <View style={styles.ratingRow}>{renderStars(item.rating)}</View>
          <Text style={styles.author}>{item.authorName || item.customerName || 'Користувач'}</Text>
        </View>
        ...
        {isLoading ? (
          ...
          <View style={styles.actions}>
            {tab === 'pending' && (
              <>
                <TouchableOpacity style={[styles.actionBtn, styles.approveBtn]} onPress={() => doAction(item.reviewId, adminApproveProductReview)}>
                  <Ionicons name="checkmark" size={16} color="#fff" />
                  <Text style={styles.actionText}>Схвалити</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.actionBtn, styles.rejectBtn]} onPress={() => doAction(item.reviewId, adminRejectProductReview)}>
                  <Ionicons name="close" size={16} color="#fff" />
                  <Text style={styles.actionText}>Відхилити</Text>
                </TouchableOpacity>
              </>
            )}
            <TouchableOpacity style={[styles.actionBtn, styles.deleteBtn]} onPress={() => handleDelete(itemId)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Відгуки</Text>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Text style={[styles.tabText, tab === 'pending' && styles.tabTextActive]}>На перевірці</Text>
        </TouchableOpacity>
        <TouchableOpacity
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>Відгуків немає</Text>}
        />
      )}
    </SafeAreaView>
  );
