...
import { adminGetCategories, createProduct, createOffer, uploadProductImages } from '../../../shared/api';
...
export default function SellerAddProduct() {
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') { Alert.alert('Немає доступу', 'Дозвольте доступ до галереї'); return; }
  ...
  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('Помилка', 'Введіть назву товару'); return; }
    if (!selectedCategory) { Alert.alert('Помилка', 'Оберіть категорію'); return; }
    if (!price || isNaN(Number(price)) || Number(price) <= 0) { Alert.alert('Помилка', 'Введіть коректну ціну'); return; }
    if (!stock || isNaN(Number(stock)) || Number(stock) < 0) { Alert.alert('Помилка', 'Введіть кількість'); return; }
    ...
    try {
      ...
      Alert.alert('Готово', 'Товар успішно додано!', [
        { text: 'OK', onPress: () => router.back() }
      ]);
    } catch (e) {
      const msg = e?.response?.data?.detail || e?.response?.data?.title || 'Помилка при створенні товару';
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Новий товар</Text>
        <TouchableOpacity onPress={handleSave} disabled={saving} style={styles.saveBtn}>
          {saving ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.saveBtnText}>Зберегти</Text>}
        </TouchableOpacity>
      </View>

      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">

        <Text style={styles.label}>Фото товару</Text>
        <TouchableOpacity style={styles.imagePicker} onPress={pickImage}>
          {image ? (
            ...
            <View style={styles.imagePlaceholder}>
              ...
              <Text style={styles.imagePlaceholderText}>Натисніть щоб обрати фото</Text>
            </View>
          )}
        </TouchableOpacity>
        {image && (
