...
import { View, Text, StyleSheet, Image, TextInput, TouchableOpacity, ScrollView, FlatList, ActivityIndicator, Dimensions, Linking, Alert } from 'react-native';
import { useRouter, useFocusEffect } from 'expo-router';
import { Ionicons } from '@expo/vector-icons';
import { useAuth } from '../../../features/auth/AuthContext';
import { getProducts, getCategories, addToFavorites, removeFromFavorites, getCart, getFavorites } from '../../../shared/api';
...
export default function HomeScreen() {
  ...
  // загружает товары с сервера когда экран открывается
  useEffect(() => {
    if (!fontsLoaded) return;
    const load = async () => {
      try {
        const [phonesRes, categoriesRes] = await Promise.all([
          getProducts(SMARTPHONES_CATEGORY_ID),
          getCategories(),
        ]);
  ...
  // обновляет избраное и кол-во товаров в корзине каждый раз когда заходишь на екран
  useFocusEffect(
    useCallback(() => {
      if (!user) {
      ...
      getFavorites().then(res => {
        ...
        setWishlistIds(new Set(raw.map(i => i.id || i.productId)));
      }).catch(() => {});

      getCart().then(res => {
        const raw = res.data?.items || res.data || [];
        const total = raw.reduce((sum, i) => sum + (i.quantity || 1), 0);
        setCartCount(total);
      }).catch(() => {});
    }, [user])
  );

  // добавляет или убирает товар из избраного
  const toggleWishlist = async (product) => {
    ...
    if (wishlistIds.has(product.id)) {
      ...
      try { await removeFromFavorites(product.id); } catch {}
    } else {
      setWishlistIds(prev => new Set(prev).add(product.id));
      try { await addToFavorites(product.id); } catch {}
    }
  };

  if (!fontsLoaded) return null;

  // карточка товара для горизонтального списка
  function renderProductCard(product, key) {
    const img = getProductImage(product.name); // берем картинку по названию товара
    const isWished = wishlistIds.has(product.id); // проверяем есть ли в избраном
    // вся карточка кликабельна - переходим на страницу товара
    return (
      <TouchableOpacity key={key} style={styles.card} activeOpacity={0.8}
        onPress={() => router.push({ pathname: '/(tabs)/ProductDetailScreen', params: { id: product.id, type: 'api' } })}>
        {/* кнопка сердечко - если в избраном красное, если нет серое */}
        ...
        {/* если картинка найдена показываем её, если нет - заглушка */}
        {img ? (
        ...
        {/* старая цена зачеркнутая, показываем только если есть */}
  ...
  // карточка для сетки 2 колонки внизу экрана
  function renderGridCard(product, key) {
    const img = getProductImage(product.name); // берем картинку по названию товара
    const isWished = wishlistIds.has(product.id); // проверяем есть ли в избраном
    // вся карточка кликабельна - переходим на страницу товара
    return (
      <TouchableOpacity key={key} style={styles.gridCard} activeOpacity={0.8}
        onPress={() => router.push({ pathname: '/(tabs)/ProductDetailScreen', params: { id: product.id, type: 'api' } })}>
        {/* кнопка сердечко - если в избраном красное, если нет серое */}
        <TouchableOpacity style={styles.heartBtn} onPress={() => toggleWishlist(product)}
          hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={() => router.push('/(tabs)/catalog')}>
          <View pointerEvents="none">
            <TextInput style={styles.search} placeholder="Я шукаю..." placeholderTextColor="#ccc" editable={false} />
          </View>
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 20 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ paddingHorizontal: 8, paddingBottom: 4, gap: 2 }} style={{ marginBottom: 8 }}>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', width: 150, marginHorizontal: 2, padding: 10, borderRadius: 12, position: 'relative', borderWidth: 0.5, borderColor: 'rgba(166,165,165,1)', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 2 },
  ...
  supportBtn: { width: 258, height: 36, borderRadius: 41, backgroundColor: '#98be2a', alignSelf: 'center', justifyContent: 'center', alignItems: 'center', marginTop: 24, marginBottom: 16 },
