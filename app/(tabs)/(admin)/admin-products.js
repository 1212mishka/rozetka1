...
import { adminGetCategories, getProducts, getProduct } from '../../../shared/api';
...
export default function AdminProducts() {
  ...
  useEffect(() => {
    adminGetCategories()
      ...
      .catch(() => Alert.alert('Помилка', 'Не вдалося завантажити категорії'))
  ...
  if (view === 'categories') {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.hint}>Оберіть категорію</Text>
        {loadingCats ? (
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loadingProds ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>Товарів немає</Text>}
        />
      )}
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {loadingDetail ? (
                ...
                <View style={styles.noImageBox}>
                  ...
                  <Text style={styles.noImageText}>Зображення відсутнє</Text>
                  <Text style={styles.noImageHint}>Зображення завантажують продавці через свій акаунт</Text>
                </View>
              )}
              ...
              {!loadingDetail && (
                <>
                  <Text style={[styles.sectionLabel, { marginTop: 16 }]}>Інформація</Text>
                  <View style={styles.infoTable}>
                    {price != null && (
                      <InfoRow label="Ціна" value={`${Number(price).toLocaleString('uk-UA')} ₴`} accent />
                    )}
                    {oldPrice != null && (
                      <InfoRow label="Стара ціна" value={`${Number(oldPrice).toLocaleString('uk-UA')} ₴`} />
                    )}
                    {stock != null && (
                      <InfoRow label="В наявності" value={`${stock} шт.`} />
                    )}
                    {detail?.description && (
                      <InfoRow label="Опис" value={detail.description} />
                    )}
                  </View>
                </>
              )}
