...
import { getProfile, logout as logoutApi } from '../../../shared/api';
export default function ProfileScreen() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    getProfile().then(res => {
      ...
      setDisplayName(name);
    }).catch(() => setDisplayName(user.email || 'Гість'));
  }, [user]));

  if (!fontsLoaded) return null;

  const menuGroup1 = [
    { icon: 'gift-outline',         label: 'Мій бонусний рахунок',        badge: null, onPress: null },
    { icon: 'diamond-outline',      label: 'Підписка Premium',            badge: null, onPress: null },
    { icon: 'receipt-outline',      label: 'Мої замовлення',              badge: null, onPress: () => router.push('/(tabs)/orders') },
    { icon: 'wallet-outline',       label: 'Мій гаманець',                badge: null, onPress: null },
    { icon: 'shield-outline',       label: 'Сервіс та повернення товару', badge: null, onPress: null },
    { icon: 'mail-outline',         label: 'Моє листування',              badge: 1,    onPress: null },
    { icon: 'git-compare-outline',  label: 'Порівняння',                  badge: 1,    onPress: null },
  ];

  const menuGroup2 = [
    { icon: 'storefront-outline',          label: 'Адреси та години роботи', badge: null },
