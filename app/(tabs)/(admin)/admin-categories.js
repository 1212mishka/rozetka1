...
export default function AdminCategories() {
  ...
  const handleCreate = async () => {
    if (!newName.trim()) { Alert.alert('Помилка', 'Введіть назву'); return; }
    setCreating(true);
    try {
      ...
      Alert.alert('Помилка', e?.response?.data?.detail || 'Помилка при створенні');
    } finally {
      setCreating(false);
    }
  };
  ...
  const handlePickImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Немає доступу', 'Дозвольте доступ до галереї в налаштуваннях');
      return;
    }
    ...
    try {
      ...
      setCatDetail(res.data);
      load();
      Alert.alert('Готово', 'Зображення завантажено');
    } catch (e) {
      Alert.alert('Помилка', e?.response?.data?.detail || 'Не вдалося завантажити зображення');
    } finally {
  ...
  const handleDeleteCategory = () => {
    Alert.alert(
      'Видалити категорію?',
      `Категорія "${selectedCat?.name}" буде видалена назавжди. Цю дію не можна скасувати.`,
      [
        { text: 'Скасувати', style: 'cancel' },
        {
          text: 'Видалити', style: 'destructive', onPress: async () => {
            setImageLoading(true);
            try {
              await adminDeleteCategory(selectedCat.id);
              setManageVisible(false);
              load();
            } catch (e) {
              Alert.alert('Помилка', e?.response?.data?.detail || 'Не вдалося видалити категорію');
            } finally {
              setImageLoading(false);
            }
          }
  ...
  const handleDeleteImage = () => {
    Alert.alert('Видалити зображення?', 'Цю дію не можна скасувати', [
      { text: 'Скасувати', style: 'cancel' },
      {
        text: 'Видалити', style: 'destructive', onPress: async () => {
          setImageLoading(true);
          try {
            ...
            Alert.alert('Помилка', e?.response?.data?.detail || 'Не вдалося видалити зображення');
          } finally {
            setImageLoading(false);
          }
        }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Категорії</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>Категорій немає</Text>}
        />
      )}
          ...
          <View style={styles.modal}>
            ...
            {imageLoading ? (
              ...
              <View>
                ...
                <View style={styles.imageActions}>
                  <TouchableOpacity style={styles.imageBtn} onPress={handlePickImage}>
                    ...
                    <Text style={[styles.imageBtnText, { color: '#4A90D9' }]}>Замінити</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.imageBtn, { borderColor: '#E74C3C' }]} onPress={handleDeleteImage}>
                    <Ionicons name="trash-outline" size={18} color="#E74C3C" />
                    <Text style={[styles.imageBtnText, { color: '#E74C3C' }]}>Фото</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ) : (
              <TouchableOpacity style={styles.uploadArea} onPress={handlePickImage}>
                ...
                <Text style={styles.uploadText}>Натисніть щоб завантажити зображення</Text>
              </TouchableOpacity>
            )}
            ...
            <TouchableOpacity style={styles.deleteCatBtn} onPress={handleDeleteCategory}>
              <Ionicons name="trash-outline" size={18} color="#E74C3C" />
              <Text style={styles.deleteCatText}>Видалити категорію</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
          ...
          <View style={styles.modal}>
            ...
            <View style={styles.modalBtns}>
              ...
              <TouchableOpacity style={styles.confirmBtn} onPress={handleCreate} disabled={creating}>
                {creating ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.confirmText}>Створити</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  deleteCatBtn: {
    ...
    gap: 8, borderWidth: 1.5, borderColor: '#E74C3C', borderRadius: 10,
    paddingVertical: 12,
  },
  deleteCatText: { fontFamily: 'Montserrat_500Medium', fontSize: 14, color: '#E74C3C' },
});
